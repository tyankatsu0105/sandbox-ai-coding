...
各作業は Plan（計画）、Do（実行）、Check（確認）、Action（改善）のサイクルで進めます。
以下の点を意識して行動してください：
