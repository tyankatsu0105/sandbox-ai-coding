...
- TS の型から引数や返却値が推測できる場合は、それをコメントに含めないでください。
- コメントはコードの意図や背景を説明するために使用し、自明なコードには不要です。
...
- ただし、早期リターンが多すぎると逆に混乱を招く可能性があるため、適切なバランスを保つことが重要です。
...
- これらのコメントは、実際に修正するまでの一時的なものであることを明確にしてください。
- 抽象的なコメントではなく、具体的な内容を記述することで、他の開発者が理解しやすくなります。
